...
var targetEventTypes = ['focus', 'blur', 'detachedvisiblenode'];
...
function dispatchFocusWithinEvents(context, event, props, state) {
  ...
  if (isFunction(onFocusWithin)) {
    var syntheticEvent = createFocusEvent(context, 'focuswithin', target, pointerType);
  ...
  onEvent: function (event, context, props, state) {
    ...
    switch (type) {
      ...
      case 'detachedvisiblenode':
        {
          var onDetachedVisibleNode = props.onDetachedVisibleNode;

          if (isFunction(onDetachedVisibleNode)) {
            var syntheticEvent = createFocusEvent(context, 'detachedvisiblenode', event.target, state.pointerType);
            context.dispatchEvent(syntheticEvent, onDetachedVisibleNode, DiscreteEvent);
          }
        }
    }
  },
