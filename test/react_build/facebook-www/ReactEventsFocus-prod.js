...
var React = require("react"),
  ...
  isMac =
    "undefined" !== typeof window && null != window.navigator
      ...
      : !1,
  targetEventTypes = ["focus", "blur", "detachedvisiblenode"],
  rootEventTypes =
...
function dispatchBlurEvents(context, props, state) {
  var pointerType = state.pointerType,
    ...
    onBlur = props.onBlur;
  isFunction(onBlur) &&
    ((pointerType = createFocusEvent(context, "blur", target, pointerType)),
      ...
      switch (type) {
        case "focus":
          ...
          state.isFocused ||
            ...
            (event = props.onFocus),
            isFunction(event) &&
              ((type = createFocusEvent(context, "focus", target, type)),
    ...
    if (props.disabled)
      ...
      switch (type) {
        case "focus":
          ...
          type = state.pointerType;
          state = state.focusTarget || event.target;
          props = props.onFocusWithin;
          isFunction(props) &&
            ((state = createFocusEvent(context, "focuswithin", state, type)),
        ...
        case "blur":
          state.isFocused &&
            ...
            isFunction(props) &&
              ((event = createFocusEvent(context, "blurwithin", event, type)),
              context.dispatchEvent(event, props, 0)),
            (state.isFocused = !1));
          break;
        case "detachedvisiblenode":
          (props = props.onDetachedVisibleNode),
            isFunction(props) &&
              ((state = createFocusEvent(
                context,
                "detachedvisiblenode",
                event.target,
                state.pointerType
